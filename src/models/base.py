...
class Base(DeclarativeBase):
  __table_args__ = {'mysql_default_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_general_ci'}
    ...
    for k, v in filters.items():
      if '__' in k:
        ...
        if op == 'gte':
          ...
          exps.append(col.like(v))
        elif op == 'ilike':
          exps.append(col.ilike(f'%{v}%'))
        elif op == 'date':
          exps.append(func.date(col) == v if isinstance(v, date) else date.fromisoformat(v))
        elif op == 'notnull':
          exps.append(col.isnot(None))
        elif op == 'isnull':
          exps.append(col.is_(None))
      else:
        simple[k] = v
    return simple, exps
